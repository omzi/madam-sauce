...
const parseCookie = cookie => {
  if (cookie) {
    return cookie.split(';').map(value => value.split('='))
    .reduce((acc, value) => {
      acc[decodeURIComponent(value[+[]].trim())] = decodeURIComponent(value[+!![]].trim());
      return acc;
    }, {});
  }
  return {}
...
const cart = new Cart(parseCookie(document.cookie).cart ? JSON.parse(parseCookie(document.cookie).cart) : []);
